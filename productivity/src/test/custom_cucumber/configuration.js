      ...
      var formatters = formats.map(function (format) {
        ...
        switch (format.type) {
          ...
          default:
            ...
            var formatterPath;
            //var t = format.type.indexOf(":");
            //console.log("index : " + t);

            if (format.type.indexOf(":") > -1) {
              formatterPath = path.resolve(format.type);
            } else {
              var p = path.join(process.cwd(), format.type);
              formatterPath = path.resolve(p);
            }
            console.log("Formatter path: " + formatterPath);
            if (fs.existsSync(formatterPath)) {
              var formatterFactory = require(formatterPath);
              var formatter = formatterFactory(formatterOptions, Cucumber);
              return formatter;
            } else {
